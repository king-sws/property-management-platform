...
// components/payments/payments-table-integrated.tsx
"use client";
...
export function PaymentsTable({ payments, role = "LANDLORD" }: PaymentsTableProps) {
  ...
  const getStatusBadge = (status: string, dueDate?: string) => {
    const isOverdue = dueDate && status === "PENDING" && new Date(dueDate) < new Date();
    ...
    switch (status) {
      ...
      case "PENDING":
        return (
          <Badge variant="secondary" className="bg-yellow-100 text-yellow-800 dark:bg-yellow-950/20 dark:text-yellow-300">
            <Clock className="mr-1 h-3 w-3" />
            Pending
          </Badge>
        );
        ...
        <Table>
          ...
          <TableBody>
            {payments.map((payment) => (
              <TableRow
                key={payment.id}
                className="cursor-pointer hover:bg-muted/50 dark:border-gray-800"
              ...
              >
                ...
                <TableCell>
                  {payment.dueDate ? (
                    <div
                      className={`text-sm ${
                        isOverdue(payment) ? "text-red-600 dark:text-red-400 font-medium" : ""
                      }`}
                    >
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-2">
                    {/* TENANT: Show Pay Now button for pending payments */}
                    {role === "TENANT" && payment.status === "PENDING" && (
                      <Button
                        size="sm"
                        onClick={(e) => {
                          e.stopPropagation();
                          router.push(`/dashboard/payments/pay/${payment.id}`);
                        }}
                      >
                      ...
                      <DropdownMenuContent align="end">
                         ...
                         role === "LANDLORD" && (
                          <>
                            ...
                            <DropdownMenuItem
                              ...
                              className="text-green-600 focus:text-green-600 dark:text-green-500"
                            >
                        ...
                        {payment.status === "COMPLETED" && !payment.receiptUrl && (
                          <DropdownMenuItem
                            onClick={(e) => {
                              e.stopPropagation();
                              toast.info("Generate receipt feature coming soon");
                            }}
                          >
                            <Download className="mr-2 h-4 w-4" />
                            Generate Receipt
